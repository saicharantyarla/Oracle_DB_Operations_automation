"""
	Name: Start_Instance_Without_Sudo_Brakes.py
	Description: To start instance on remote or local system
...
try:
	...
	####################################################### Command to Start Instance ####################################################
			...
			command1="srvctl start instance -d %s -i %s -o open" %(Sid,Instance)
			...
			Error=stderr.read()
			#print Error
			log.info("If any error is %s "%Error)
			if "ERROR" or "unable" in output2:
				print "ExitCode: 1"
				print "ExitDesc: Error, failed to Start Instance \n %s" %(output2)
				log.info('Exitcode: 1')
				log.info('Error: {0}'.format(output2))
				Status="Error Occurred. Check Logs"
			elif "has already been started" in output:
				print "ExitCode: 0"
				print "ExitDesc: Instance %s has already been started"%(Instance)
				log.info('ExitCode: 0')
				log.info('ExitDesc: Instance %s has already been started'%(Instance))
				Status="Instance already started"	
			else:
				print "ExitCode: 0"
				print "Exitdesc: Successfully started the instance"
				log.info('ExitCode:0')
				log.info('Info: Successfully started the instance')
				Status="Successfully started instance"
...
SUBJECT = 'START INSTANCE PROCESS'
body ="""
<html>
