"""
	Name: Disable_Archive_Log_Test_Without_Sudo_Brakes.py
	Description: To disable archive log mode, remote target server or local target server
...
try:
	...
	logfile = "Disable_Archive_Log.log"
...
try:
    ...
    if len(sys.argv) == 6:
		...
		if Execution_Location=="REMOTE":
			...
			command3="ALTER DATABASE NOARCHIVELOG;"
			...
			if "ERROR" not in output4 and "ORA-" not in output4 and "SP-" not in output4:
				log.info('ExitCode: 0')
				log.info('ExitDesc: Archive Log Successfully Disabled')
				print "ExitCode: 0"
				print "ExitDesc: Archive Log Successfully Disabled"
				Status="Archive Log Disabled"
			else:
				log.info('ExitCode: 1')
				log.info('ExitDesc: Archive Log Disabling Failed')
				print "ExitCode: 0"
				print "ExitDesc: Archive Log Disabling Failed"
				Status="Archive Log Disabling Failed"
		...
		############################################## Query to Disable the Archive Log ###########################################
...
SUBJECT = 'DISABLE ARCHIVE LOG PROCESS '
body ="""
<html>
